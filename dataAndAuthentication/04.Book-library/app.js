...
async function editBook(formData, id) {
    ...
    let url = `http://localhost:3030/jsonstore/collections/books/${id}`;
    let editRersponse = await fetch(url, {
        method: 'PUT',
        headers: {
    ...
    let editResult = await editRersponse.json();
    ...
    //booksTableBody.appendChild(editedHtmlBook);
}
...
async function createBook(formData) {
    ...
    let createResult = await rersponse.json();
